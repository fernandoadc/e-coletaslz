...
function CollectorDashboard() {
  ...
  const startCollection = (pointId) => {
    setCollectionPoints((prev) =>
      prev.map((point) =>
        point.id === pointId ? { ...point, status: "in_progress" } : point,
      ),
    );
  ...
  const completeCollection = (pointId) => {
    setCollectionPoints((prev) =>
      prev.map((point) =>
        point.id === pointId ? { ...point, status: "completed" } : point,
      ),
    );
  ...
  const stats = {
    ...
    inProgress: collectionPoints.filter((p) => p.status === "in_progress")
      .length,
    completedToday: collectionPoints.filter((p) => p.status === "completed")
      .length,
    totalWeight: collectionPoints
      .filter((p) => p.status === "completed")
      .reduce((acc, point) => acc + parseInt(point.estimatedWeight), 0),
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-blue-700">
                  {currentRoute.destination} • {currentRoute.distance} •{" "}
          ...
          <div className="h-64 bg-gradient-to-br from-blue-100 to-green-100 relative flex items-center justify-center border-b">
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500">
                {filteredPoints.filter((p) => p.status === "ready").length}{" "}
          ...
          <div className="p-6 space-y-4">
              ...
              .map((point) => (
                ...
                >
                  <div className="flex items-center justify-between flex-wrap gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center flex-wrap gap-x-4 gap-y-1 text-xs text-gray-500 mt-1">
                          ...
                          <span>
                            Alerta há:{" "}
                            {Math.floor(
                              (Date.now() - point.alertTime.getTime()) / 60000,
                            )}{" "}
                            min
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                      <div
                        className={`px-2 py-1 rounded-full text-xs font-medium border ${getPriorityColor(point.priority)}`}
                      >
                        {getPriorityText(point.priority)}
                      </div>

                      <div
                        className={`px-2 py-1 rounded-full text-xs font-medium ${point.status === "ready" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                      >
                        {point.status === "ready"
                          ? "Disponível"
                          : "Em Andamento"}
            ...
            {filteredPoints.filter((p) => p.status === "ready").length ===
              ...
              <div className="text-center py-8 text-gray-500">
                ...
                <p className="text-sm">
                  Novos pontos aparecerão aqui quando os estabelecimentos
                  marcarem recipientes como cheios
                </p>
              </div>
            )}
          </div>
